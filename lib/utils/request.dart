import 'dart:async';
import 'package:all_universe_flutter/model/api_response.dart';
import 'package:dio/dio.dart';
import 'package:cookie_jar/cookie_jar.dart';
import 'package:dio_cookie_manager/dio_cookie_manager.dart';
import 'package:flutter_easyloading/flutter_easyloading.dart';
import 'package:all_universe_flutter/config.dart';
...
class Request {
  ...
  Request._internal() {
    ...
    dio.interceptors.add(InterceptorsWrapper(onRequest: (options, handler) {
      ...
      // 错误提示
      EasyLoading.showToast(eInfo.message.toString());
      // 错误交互处理
      switch (eInfo.code) {
        case 401: // 没有权限 重新登录
  ...
  Future<ResultData> get(String path,
    ...
    var response = await dio.get(path,
        queryParameters:
            params != null ? new Map<String, dynamic>.from(params) : {},
  ...
  /// restful post 操作
  Future<ResultData> post(String path,
      {dynamic params, Options? options}) async {
    Options requestOptions = options ?? Options();
  ...
  /// restful put 操作
  Future<ResultData> put(String path,
      {dynamic params, Options? options}) async {
    Options requestOptions = options ?? Options();
  ...
  /// restful patch 操作
  Future<ResultData> patch(String path,
      {dynamic params, Options? options}) async {
    Options requestOptions = options ?? Options();
  ...
  /// restful delete 操作
  Future<ResultData> delete(String path,
      {dynamic params, Options? options}) async {
    Options requestOptions = options ?? Options();
  ...
  Future<ResultData> postForm(String path,
      {dynamic params, Options? options}) async {
    Options requestOptions = options ?? Options();
